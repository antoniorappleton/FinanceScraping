...
DETECT_PATTERNS = {
    'crypto': r'^[A-Z]+-[A-Z]+$',  # BTC-USD, ETH-BTC
    'etf': r'\.(E[ST]|A[CT])\b',   # .DE .ES .PA etc with ETF-like, but heuristic
}
...
def detect_asset_type_and_market(ticker: str) -> Tuple[str, str]:
    ...
    # ETF heuristic: EU/PT + certain patterns or default to stock
    if market in ['EU', 'PT'] and re.search(DETECT_PATTERNS['etf'], ticker_upper):
        return 'etf', market
...
def select_sources(asset_type: str, market: str) -> List[str]:
    ...
    rules = {
        ...
        ('etf', 'EU'): ['justetf', 'yahoo', 'google_finance'],
        ('etf', 'US'): ['finviz', 'yahoo'],
        ('crypto', 'GLOBAL'): ['yahoo', 'google_finance'],
    }
    key = (asset_type, market)
    return [s for s in rules.get(key, ['yahoo']) if s in SCRAPER_REGISTRY]
...
def consolidate_results(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    consolidated = {
        'sources_used': [r['source'] for r in results if r.get('source')],
